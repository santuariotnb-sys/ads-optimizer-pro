...
function SaturationGauge({ percent }: { percent: number }) {
  ...
  const color = percent >= 80 ? '#ef4444' : percent >= 50 ? '#f59e0b' : '#84cc16';

  return (
    <svg width={72} height={72} style={{ display: 'block' }}>
      <circle cx={36} cy={36} r={radius} fill="none" stroke="rgba(255,200,120,0.06)" strokeWidth={stroke} />
      <circle
      ...
      <text x={36} y={36} textAnchor="middle" dominantBaseline="central"
        style={{ fill: color, fontSize: 13, fontWeight: 700, fontFamily: "'Sora', sans-serif" }}>
        {percent}%
      </text>
    </svg>
  );
...
function StatusBadge({ status, frequency }: { status: string; frequency: number }) {
  ...
  const map: Record<string, { color: string; label: string }> = {
    active: { color: '#84cc16', label: 'Ativo' },
...
function OverlapBar({ percent }: { percent: number }) {
  const color = percent > 30 ? '#ef4444' : percent > 15 ? '#f59e0b' : '#84cc16';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div style={{ flex: 1, height: 6, borderRadius: 3, background: 'rgba(255,200,120,0.06)', overflow: 'hidden' }}>
        <div style={{
          width: `${percent}%`, height: '100%', borderRadius: 3, background: color,
          transition: 'width 0.4s ease', boxShadow: `0 0 8px ${color}60`,
        }} />
      </div>
      <span style={{ fontSize: 12, fontWeight: 600, fontFamily: "'Sora', sans-serif", color, minWidth: 36 }}>
        {percent}%
      </span>
    </div>
  );
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            {overlappingPairs.map((pair, i) => (
              <div key={i} style={{ fontSize: 12, color: '#fafaf9', opacity: 0.8 }}>
      ...
      <div style={{
        ...
        {mockAudiences.map((aud) => (
          <div
            ...
            style={{
              ...
              borderColor: hoveredCard === aud.id ? 'rgba(255,200,120,0.14)' : 'rgba(255,200,120,0.06)',
              boxShadow: hoveredCard === aud.id ? '0 0 30px rgba(245,158,11,0.08), 0 8px 32px rgba(0,0,0,0.3)' : '0 1px 0 0 rgba(255,200,120,0.04) inset, 0 -1px 0 0 rgba(0,0,0,0.2) inset, 0 4px 16px rgba(0,0,0,0.4), 0 12px 40px rgba(0,0,0,0.25)',
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#fafaf9', marginBottom: 4, fontFamily: "'DM Sans', sans-serif" }}>
                  {aud.name}
                </div>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 10, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>CPA</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#fafaf9', fontFamily: "'Sora', sans-serif" }}>
                  {formatCurrency(aud.cpa)}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>ROAS</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#fafaf9', fontFamily: "'Sora', sans-serif" }}>
                  {aud.roas.toFixed(1)}x
                </div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Freq.</div>
                <div style={{
                  fontSize: 16, fontWeight: 700, fontFamily: "'Sora', sans-serif",
                  color: aud.frequency > 2.5 ? '#ef4444' : '#fafaf9',
                }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 10, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
                Overlap
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 10, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>
                  Saturacao
                </div>
        ...
        {mockAudiences.length === 0 && (
          <div style={{ ...glassCard, padding: 40, textAlign: 'center' }}>
            <div style={{ fontSize: 14, color: '#a8a29e' }}>Nenhuma audiencia encontrada</div>
          </div>
        )}
      </div>
    </div>
